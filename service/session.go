...
func (c *sessionService) GetSessionByEmail(email string) (model.Session, error) {
	// Gunakan SessionAvailEmail dari repositori
	session, err := c.sessionRepo.SessionAvailEmail(email)
	if err != nil {
		return model.Session{}, err // Jika terjadi kesalahan, kembalikan error
	}
	return session, nil // Kembalikan sesi jika berhasil
}
